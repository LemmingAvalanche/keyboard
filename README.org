...
- Go to the first group (“home”): ~Ext+W~.
- Go to the next group: ~Ext+E~.
- Go to the previous group: ~Ext+R~.
- Go to the last group (“end”): ~Ext+T~.

If you are only using two group it is probably easier to just use “home”
(~Ext+W~) and “end” (~Ext+T~), since then you won’t keep have to keep
which group you are on in your head.
